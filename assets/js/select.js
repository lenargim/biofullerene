$(document).ready(function () {
  ...
  $('.country_select').each(function() {
    ...
    // Insert a styled div to sit over the top of the hidden select element
    $this.after('<div class="country-select"></div>');

    // Cache the styled div
    var $styledSelect = $this.next('div.country-select');

    // Show the first select option in the styled div
    let renderedOption = $this.find(":selected");
    let renderedOptionIndex = renderedOption.index();
    $styledSelect.text(renderedOption.text());
    ...
    // Insert a list item into the unordered list for each select option
    for (var i = 0; i < numberOfOptions; i++) {
      let liClass = i === renderedOptionIndex ? 'checked' : '';
      $('<li />', {
        text: $this.children('option').eq(i).text(),
        rel: $this.children('option').eq(i).val(),
        class: liClass,
  ...
  $('.state_select').each(function() {
    ...
    $listItems.on('click', function(e) {
      ...
      $(this).attr('rel') ?
          wrapper.addClass('validated').removeClass('invalid') :
          wrapper.addClass('invalid').removeClass('validated');
    });
